...
public class Odometer extends Thread {
	...
	public void run() {
		...
		while (true) {
			...
			distL = Math.PI * NavigationLab.WHEEL_RADIUS * (nowTachoL - leftMotorTachoCount) / 180;
			distR = Math.PI * NavigationLab.WHEEL_RADIUS * (nowTachoR - rightMotorTachoCount) / 180;
			leftMotorTachoCount = nowTachoL;
			rightMotorTachoCount = nowTachoR;
			deltaD = 0.5 * (distL + distR);
			deltaT = (distL - distR) / NavigationLab.TRACK;
