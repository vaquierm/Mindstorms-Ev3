...
public class BangBangController implements UltrasonicController {
	...
	public BangBangController(int bandCenter, int bandwidth, int motorLow, int motorHigh) {
		...
		WallFollowingLab.leftMotor.setSpeed(motorLow); // Start robot moving
														// forward
	...
	public void processUSData(int distance) {
		...
		if (distance >= 100 && filterControl < FILTER_OUT) {
			...
			filterControl++;
			this.distance = bandCenter; // while waiting for filter control to
										// confirm that there is indeed nothing
										// there, the robot goes straight
		...
		if (Math.abs(this.distance - bandCenter) < bandwidth) { // Sweet spot
			...
			if (this.distance < 10) { // if the robot is way way too close it
										// backs up fast
				...
				spinning = true;
			} else if (this.distance < 20) { // if the robot is way too close,
												// it spins
